...
def get_valid_url(text_to_search: str, regex: str) -> str:
    """
    Return a valid url matching the regex from the text
    """
    matches = re.findall(regex, text_to_search)
    for url in matches:
        try:
            response = requests.get(url)
            response.raise_for_status() # Raise exception if invalid url
            return url
...
max_results = st.sidebar.number_input(
    'Max search results', value=100, min_value=1, max_value=10000
)
...
if len(parsed_results) > 0:
    ...
    st.write(f'Numbrr of results with github links: {len(df) - num_github}')
    csv = convert_df(df)

    st.download_button(
        'Press to Dowload', csv, 'arxiv.csv', 'text/csv', key='download-csv'
    )
